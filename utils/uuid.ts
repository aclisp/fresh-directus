...
export function uuidToBase64(
  ...
  let base64 = "";
  const encodings = encodingCharSet;

  const bytes = new Uint8Array(16);
  let bytesIndex = 0;
  uuid.replace(/[a-fA-F0-9]{2}/g, (hex: string): string => {
    bytes[bytesIndex++] = parseInt(hex, 16);
    return "";
  });
  ...
  if (byteRemainder == 1) {
    ...
    base64 += encodings[a] + encodings[b] + (noPadding ? "" : "==");
...
export function uuidToBase64Native(uuid: string): string {
  let binary = "";
  const bytes = new Uint8Array(16);
